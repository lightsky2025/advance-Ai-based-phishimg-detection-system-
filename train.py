...
def generate_synthetic_email_data(n_samples: int = 1000) -> Tuple[pd.DataFrame, pd.Series, List[str]]:
    """Generate synthetic email phishing dataset with realistic variability and class overlap"""
    np.random.seed(TRAINING_CONFIG["random_state"])
    
    # Mixed templates - some phishing can look legitimate and vice versa
    # This creates realistic class overlap that prevents 100% accuracy
    ...
    phishing_subjects = [
        ...
        "Important: Confirm your identity now",
        ...
        # More subtle phishing that looks legitimate
        "Meeting reminder for tomorrow",
        "Document review requested",
        "Please review the attached file",
        "Quick question about the project",
        "Update on the quarterly report",
        "Team lunch next Friday",
        "Your timesheet needs approval",
    ...
    legitimate_subjects = [
        ...
        "New hire onboarding info",
        "IT maintenance scheduled",
        "Holiday schedule update",
        "Performance review meeting",
        # Some legitimate emails with urgent language (creates overlap)
        "URGENT: Server maintenance tonight",
        "Action Required: Update your password",
        "Important: Security patch deployment",
        "Alert: System will be down Saturday",
        "Verify your timesheet by Friday",
        "Please confirm your availability",
        "Final reminder: Survey due tomorrow",
        "Important update: Office relocation",
        "Security training mandatory",
        "Review needed: New company policy",
    ]
    ...
    phishing_bodies = [
        ...
        "URGENT: Your account is at risk. Please verify your identity at our secure portal.",
        "We detected unusual activity on your account. Please login to verify.",
        ...
        "Action required: Please confirm your identity to avoid account suspension.",
        "Congratulations! You've been selected for a special reward. Claim here.",
        "Limited time offer: Get 50% off your next purchase. Shop now!",
        # More subtle phishing that looks like legitimate work emails
        "Hi team, I've attached the updated report for tomorrow's meeting. Please review.",
        "Quick reminder about the client presentation next week. Let me know if you have questions.",
        "I've shared the document with you. Please add your comments by Friday.",
        "Can you review the attached spreadsheet and send me your feedback?",
        "The IT department has scheduled maintenance for this weekend. Please save your work.",
        "New company policy regarding remote work has been updated. Please review the attached.",
        "Training session on the new software will be held next Tuesday at 2 PM.",
        "Please complete the employee satisfaction survey by end of week.",
        "The budget for Q4 has been finalized. See attached for details.",
        "Your timesheet for last week needs to be resubmitted. Please correct and resend.",
    ]
    ...
    legitimate_bodies = [
        ...
        "Thanks for your help on the project. Great work everyone!",
        "Let's schedule a meeting to discuss the new feature requirements.",
        "I've updated the documentation. Please let me know if anything is unclear.",
        "The client meeting went well. Here are the action items.",
        "Please review the attached design mockups and provide feedback.",
        "Don't forget to submit your expense reports by Friday.",
        "The new office space is ready. Moving day is next Monday.",
        # Some legitimate emails that might look suspicious (creates overlap)
        "URGENT: Please update your VPN password by end of day. IT Security.",
        "Action Required: Complete your security training module by Friday.",
        "Alert: Your account will be locked after 3 failed login attempts.",
        "Important: Please verify your direct deposit information in HR portal.",
        "Security notice: Please enable two-factor authentication on your account.",
        "Your password expires in 3 days. Please reset it at the IT portal.",
        "Verification required: Please confirm your email address for the new system.",
        "Important update: New security protocols effective immediately.",
        "Please review and confirm the updated terms of service.",
        "Account maintenance: System upgrade scheduled for this weekend.",
    ]
    
    # Mixed domains - some phishing uses legitimate-looking domains
    phishing_domains = [
        "secure-verify.xyz", "account-security.tk", "billing-update.ml",
        "auth-portal.cf", "payment-fix.ga", "verify-account.pw",
        "security-alert.cc", "update-now.top", "confirm-id.club",
        # Phishing that uses more legitimate-looking domains (overlap)
        "company-security.com", "hr-portal.net", "it-department.org",
        "internal-verify.com", "corp-security.net", "employee-portal.org",
        "secure-company.com", "internal-hr.net", "company-update.org",
    ]
    
    legitimate_domains = [
        "company.com", "corp.example.com", "internal.company.com",
        "hr.company.com", "it.company.com", "team.company.com",
        "dept.company.com", "office.company.com", "work.company.com",
        # Some legitimate emails from external domains (overlap)
        "gmail.com", "outlook.com", "yahoo.com",
        "contractor-email.com", "vendor-portal.net", "partner-company.org",
    ]
    ...
    for i in range(n_samples):
        ...
        if is_phishing:
            # 70% chance of obvious phishing, 30% chance of subtle phishing
            if np.random.random() < 0.7:
                # Obvious phishing
                subject = np.random.choice(phishing_subjects[:15])  # First 15 are obvious
                body = np.random.choice(phishing_bodies[:10])  # First 10 are obvious
                domain = np.random.choice(phishing_domains[:10])  # First 10 are suspicious
            else:
                # Subtle phishing (looks like legitimate work email)
                subject = np.random.choice(phishing_subjects[15:])  # Last 10 are subtle
                body = np.random.choice(phishing_bodies[10:])  # Last 10 are subtle
                domain = np.random.choice(phishing_domains[10:])  # Last 9 look legitimate
            
            sender = f"user{np.random.randint(1,9999)}@{domain}"
            label = 1
        else:
            # 70% chance of obvious legitimate, 30% chance of urgent-looking legitimate
            if np.random.random() < 0.7:
                # Obvious legitimate
                subject = np.random.choice(legitimate_subjects[:15])  # First 15 are normal
                body = np.random.choice(legitimate_bodies[:10])  # First 10 are normal
                domain = np.random.choice(legitimate_domains[:9])  # First 9 are company domains
            else:
                # Legitimate but urgent-looking (security notices, etc.)
                subject = np.random.choice(legitimate_subjects[15:])  # Last 10 are urgent
                body = np.random.choice(legitimate_bodies[10:])  # Last 10 are urgent
                domain = np.random.choice(legitimate_domains[9:])  # Last 6 are external
            
            sender = f"user{np.random.randint(1,9999)}@{domain}"
            label = 0
        
        # Add randomness to make each email unique
        extra_text = ""
        if np.random.random() < 0.3:
            ...
            extra_text = np.random.choice(extra_phrases)
        
        body = body + extra_text
        ...
        email_data = {
            ...
            'headers': {},
            'html_content': '',
            'num_attachments': np.random.choice([0, 1, 2], p=[0.6, 0.3, 0.1])
        }
...
def generate_synthetic_url_data(n_samples: int = 1000) -> Tuple[np.ndarray, np.ndarray, List[str]]:
    """Generate synthetic URL phishing dataset with realistic class overlap"""
    np.random.seed(TRAINING_CONFIG["random_state"])
    ...
    legitimate_domains = [
        ...
        "medium.com", "wikipedia.org", "mozilla.org", "apache.org",
        "nodejs.org", "reactjs.org", "vuejs.org", "angular.io",
        "docker.com", "kubernetes.io", "nginx.com", "cloudflare.com",
    ]
    
    legitimate_paths = [
        "", "/docs", "/api", "/about", "/contact", "/blog",
        "/products", "/services", "/support", "/help", "/faq",
        "/articles", "/tutorials", "/guides", "/reference",
    ]
    
    # Phishing brand targets
    phishing_brand_targets = [
    ...
    phishing_action_words = [
        ...
        "confirm", "auth", "signin", "unlock", "restore",
    ]
    
    # TLDs - both phishing and legitimate can use .com, .net, etc.
    all_tlds = [".com", ".net", ".org", ".io", ".co", ".xyz", ".tk", ".ml", ".ga", ".cf"]
    common_tlds = [".com", ".net", ".org", ".io"]
    suspicious_tlds = [".xyz", ".tk", ".ml", ".ga", ".cf", ".pw", ".cc", ".top"]
    ...
    for i in range(n_samples):
        ...
        if is_phishing:
            # 40% obvious phishing, 60% subtle phishing that looks like legitimate
            if np.random.random() < 0.4:
                # Obvious phishing - suspicious TLDs, brand names, action words
                brand = np.random.choice(phishing_brand_targets)
                action = np.random.choice(phishing_action_words)
                tld = np.random.choice(suspicious_tlds)
                path = np.random.choice(["/login", "/verify", "/account", "/auth", "/signin", "/update", "/billing", ""])
                prefix = np.random.choice(["secure-", "verify-", "account-", "login-", ""])
                
                url = f"http://{prefix}{brand}-{action}{tld}{path}"
            else:
                # Subtle phishing - uses common TLDs, looks very similar to legitimate
                brand = np.random.choice(phishing_brand_targets)
                action = np.random.choice(phishing_action_words)
                tld = np.random.choice(common_tlds)  # Uses .com, .net, etc.
                path = np.random.choice(["", "/login", "/account", "/verify", "/auth", "/docs", "/api"])
                
                # Uses https to look legitimate
                protocol = np.random.choice(["https://", "http://"], p=[0.8, 0.2])
                
                # Sometimes no hyphen to look more legitimate
                if np.random.random() < 0.3:
                    url = f"{protocol}{brand}{action}{tld}{path}"
                else:
                    url = f"{protocol}{brand}-{action}{tld}{path}"
            
            label = 1
        else:
            # 40% obvious legitimate, 60% that might look suspicious
            if np.random.random() < 0.4:
                # Obvious legitimate - well-known domains with clean paths
                domain = np.random.choice(legitimate_domains)
                path = np.random.choice(legitimate_paths)
                protocol = "https://"
                www = np.random.choice(["www.", ""])
                
                url = f"{protocol}{www}{domain}{path}"
            else:
                # Legitimate but looks suspicious - has query params, numbers, etc.
                domain = np.random.choice(legitimate_domains)
                path = np.random.choice(["", "/login", "/account", "/verify", "/auth", "/docs"])
                
                # Add query parameters which look suspicious
                query = ""
                if np.random.random() < 0.6:
                    query = f"?id={np.random.randint(1, 9999)}&ref={np.random.randint(1, 999)}&token=abc{np.random.randint(1, 999)}"
                
                protocol = np.random.choice(["https://", "http://"], p=[0.7, 0.3])
                www = np.random.choice(["www.", ""])
                
                url = f"{protocol}{www}{domain}{path}{query}"
    ...
    for url, label in zip(urls, labels):
        try:
            ...
            # Add significant noise to features to create overlap
            noise_level = 0.08  # Higher noise to ensure class overlap
            noise = np.random.normal(0, noise_level, len(feature_values))
            feature_values = [max(0, min(1, fv + n)) for fv, n in zip(feature_values, noise)]
